...
namespace PDI_MDL_Common
{
	namespace stEleOperHelper
	{
		...
		{
			//Get Inverse transform to Origin Point;
			DisplayHandlerP dh = eeh.GetDisplayHandler();
			RotMatrix rm;
			dh->GetOrientation(eeh, rm);
			DPoint3d dp;
			dh->GetTransformOrigin(eeh, dp);
			GetTransform2Origin(eeh, outTr2Origin,
				dp, rm);
		}
		...
		{
			//Restore the element to its origin pos and rot.
			DPoint3d dOrigin = { 0,0,0 };
			//PDI_MDL_Common::stEleOperHelper::MoveAbsPos(m_eeh, m_prePoint, dOrigin);
			DVec3d      vec;
			// Setup translation from accept point to this button event point.
			vec.DifferenceOf(dOrigin, dpCur);
			Transform tran4Mov2Origin = Transform::From(vec);

			RotMatrix rotInverse = RotMatrix::FromIdentity();
			if (false == rotCur.InverseOf(rotInverse))
				mdlDialog_dmsgsPrint(L"Can not inverse the RotMatrix");
			Transform tran4Rot2Origin = Transform::From(rotInverse);

			outTr2Origin = tran4Rot2Origin * tran4Mov2Origin;
		}
